...
export class OpError extends OperationalError {
  ...
  constructor({
    message = "Generic operational error",
    code = 500,
    original = null,
  }: { message?: string; code?: number; original?: Error | null } = {}) {
    super(message);
...
export class BadRequestError extends OpError {
  errors: any;
  constructor({
    message = "Your request could not be served. Please make sure you provided the correct input.",
    code = 400,
    original = null,
    errors = [],
  }: {
